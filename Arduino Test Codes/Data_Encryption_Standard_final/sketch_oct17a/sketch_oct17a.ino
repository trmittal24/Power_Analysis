...
/*
*	implementing a macro for the addbit function
*	(first function in the function definitions below)
*	this code is being written to be run on Arduino
*	so, pointer use must be avoided (for kicks)
...
// variables to be used in main function
uint64_t key;
uint64_t data;
...
void key_schedule(int round)
{
	...
	*/
	if(round == 0)
	{
		...
		for(int ii = 0; ii < 56; ii++)
		{
			if(ii < 28)
			{
				ADD_BIT(key_left, a_key[round], PC1[ii] - 1, ii)
			}
			else
			{
				ADD_BIT(key_right, a_key[round], PC1[ii] - 1, ii % 28)
			}
		}
	...
	*/
	...
	{
		ADD_BIT(a_key[round], next_key, PC2[ii] - 1, ii)
	}
}
